...
description: >
  ...
  Use when the user asks to find skills, browse skill catalogs, install a skill for agent,
  or extend agent's capabilities with reusable AI agent components.
license: MIT
---
...
4. Install by saving files to `.agent/skills/{slug}/` and verify the SKILL.md exists
...
2. Create the directory `.agent/skills/{slug}/`
3. Save each file to the appropriate location:
   - `SKILL.md` → `.agent/skills/{slug}/SKILL.md`
   - Other files → `.agent/skills/{slug}/{filename}`
